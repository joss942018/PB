...
import { resetCaracteristicasToInit, setCaracteristicasMessage } from "redux/actions";
import { Button, Card, Col, Form, InputGroup } from "react-bootstrap";
import { Constant } from "template/Constant";
import ConfirmationModal from "template/ConfirmationModal";
import { deleteCaracteristicas } from "services/caracteristicasService";
type Props = {
    hideShowForm: (action) => void;
    handleRowEdit: (row) => void;
    getData: (page, pageSize, searchKey) => void;
};
export const CaracteristicasTable: React.FC<Props> = ({ hideShowForm, handleRowEdit, getData }) => {
    ...
    useEffect(() => {
        if (rData && rData.list && rData.list.length === 0) {
            dispatch(resetCaracteristicasToInit());
    ...
    const handleServerDelete = async () => {
        if (rowData) {
            const response = await deleteCaracteristicas(rowData.id);
            if (response) {
                dispatch(resetCaracteristicasToInit());
                dispatch(setCaracteristicasMessage("Deleted Successfully"));
                getData(Constant.defaultPageNumber, Constant.defaultPageSize, '');
                setShowDelete(false);
            } else {
...
{name: 'tipo_id', selector: row => row.tipo_id, sortable: true},
{name: 'nombre', selector: row => row.tipos_caracteristicas.nombre, sortable: true},
    ...
    return (
        <Card className="shadow mb-4">
            <Card.Header className="py-3">
                <h6 className="m-0 font-weight-bold text-primary">Caracteristicas List ({rData.totalCount})
