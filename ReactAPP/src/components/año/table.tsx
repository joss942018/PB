...
import { resetAñoToInit, setAñoMessage } from "redux/actions";
import { Button, Card, Col, Form, InputGroup } from "react-bootstrap";
import { Constant } from "template/Constant";
import ConfirmationModal from "template/ConfirmationModal";
import { deleteAño } from "services/añoService";
type Props = {
    hideShowForm: (action) => void;
    handleRowEdit: (row) => void;
    getData: (page, pageSize, searchKey) => void;
};
export const AñoTable: React.FC<Props> = ({ hideShowForm, handleRowEdit, getData }) => {
    ...
    const handleServerDelete = async () => {
        if (rowData) {
            const response = await deleteAño(rowData.id);
            if (response) {
                dispatch(resetAñoToInit());
                dispatch(setAñoMessage("Deleted Successfully"));
                getData(Constant.defaultPageNumber, Constant.defaultPageSize, '');
                setShowDelete(false);
            } else {
    ...
    return (
        <Card className="shadow mb-4">
            <Card.Header className="py-3">
                <h6 className="m-0 font-weight-bold text-primary">Año List ({rData.totalCount})
