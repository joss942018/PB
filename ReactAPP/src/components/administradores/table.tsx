...
import { resetAdministradoresToInit, setAdministradoresMessage } from "redux/actions";
import { Button, Card, Col, Form, InputGroup } from "react-bootstrap";
import { Constant } from "template/Constant";
import ConfirmationModal from "template/ConfirmationModal";
import { deleteAdministradores } from "services/administradoresService";
type Props = {
    hideShowForm: (action) => void;
    handleRowEdit: (row) => void;
    getData: (page, pageSize, searchKey) => void;
};
export const AdministradoresTable: React.FC<Props> = ({ hideShowForm, handleRowEdit, getData }) => {
    ...
    useEffect(() => {
        if (rData && rData.list && rData.list.length === 0) {
            dispatch(resetAdministradoresToInit());
    ...
    const handleServerDelete = async () => {
        if (rowData) {
            const response = await deleteAdministradores(rowData.id);
            if (response) {
                dispatch(resetAdministradoresToInit());
                dispatch(setAdministradoresMessage("Deleted Successfully"));
                getData(Constant.defaultPageNumber, Constant.defaultPageSize, '');
                setShowDelete(false);
            } else {
...
{name: 'usuario_id', selector: row => row.usuario_id, sortable: true},
{name: 'email', selector: row => row.usuarios.email, sortable: true},
    ...
    return (
        <Card className="shadow mb-4">
            <Card.Header className="py-3">
                <h6 className="m-0 font-weight-bold text-primary">Administradores List ({rData.totalCount})
