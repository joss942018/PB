...
import { resetUsuariosToInit, setUsuariosMessage } from "redux/actions";
import { Button, Card, Col, Form, InputGroup } from "react-bootstrap";
import { Constant } from "template/Constant";
import ConfirmationModal from "template/ConfirmationModal";
import { deleteUsuarios } from "services/usuariosService";
type Props = {
    hideShowForm: (action) => void;
    handleRowEdit: (row) => void;
    getData: (page, pageSize, searchKey) => void;
};
export const UsuariosTable: React.FC<Props> = ({ hideShowForm, handleRowEdit, getData }) => {
    ...
    const handleServerDelete = async () => {
        if (rowData) {
            const response = await deleteUsuarios(rowData.id);
            if (response) {
                dispatch(resetUsuariosToInit());
                dispatch(setUsuariosMessage("Deleted Successfully"));
                getData(Constant.defaultPageNumber, Constant.defaultPageSize, '');
                setShowDelete(false);
            } else {
...
{name: 'contrasena', selector: row => row.contrasena, sortable: true},
{name: 'rol', selector: row => row.rol, sortable: true},
{name: 'es_activo', selector: row => row.es_activo, sortable: true},
    ...
    return (
        <Card className="shadow mb-4">
            <Card.Header className="py-3">
                <h6 className="m-0 font-weight-bold text-primary">Usuarios List ({rData.totalCount})
