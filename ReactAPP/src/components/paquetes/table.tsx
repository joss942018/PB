...
import { resetPaquetesToInit, setPaquetesMessage } from "redux/actions";
import { Button, Card, Col, Form, InputGroup } from "react-bootstrap";
import { Constant } from "template/Constant";
import ConfirmationModal from "template/ConfirmationModal";
import { deletePaquetes } from "services/paquetesService";
type Props = {
    hideShowForm: (action) => void;
    handleRowEdit: (row) => void;
    getData: (page, pageSize, searchKey) => void;
};
export const PaquetesTable: React.FC<Props> = ({ hideShowForm, handleRowEdit, getData }) => {
    ...
    const handleServerDelete = async () => {
        if (rowData) {
            const response = await deletePaquetes(rowData.id);
            if (response) {
                dispatch(resetPaquetesToInit());
                dispatch(setPaquetesMessage("Deleted Successfully"));
                getData(Constant.defaultPageNumber, Constant.defaultPageSize, '');
                setShowDelete(false);
            } else {
...
{name: 'aseguradora_id', selector: row => row.aseguradora_id, sortable: true},
{name: 'nombre', selector: row => row.aseguradoras.nombre, sortable: true},
    ...
    return (
        <Card className="shadow mb-4">
            <Card.Header className="py-3">
                <h6 className="m-0 font-weight-bold text-primary">Paquetes List ({rData.totalCount})
