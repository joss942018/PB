...
import { resetTipos_CaracteristicasToInit, setTipos_CaracteristicasMessage } from "redux/actions";
import { Button, Card, Col, Form, InputGroup } from "react-bootstrap";
import { Constant } from "template/Constant";
import ConfirmationModal from "template/ConfirmationModal";
import { deleteTipos_Caracteristicas } from "services/tipos_caracteristicasService";
type Props = {
    hideShowForm: (action) => void;
    handleRowEdit: (row) => void;
    getData: (page, pageSize, searchKey) => void;
};
export const Tipos_CaracteristicasTable: React.FC<Props> = ({ hideShowForm, handleRowEdit, getData }) => {
    ...
    const handleServerDelete = async () => {
        if (rowData) {
            const response = await deleteTipos_Caracteristicas(rowData.id);
            if (response) {
                dispatch(resetTipos_CaracteristicasToInit());
                dispatch(setTipos_CaracteristicasMessage("Deleted Successfully"));
                getData(Constant.defaultPageNumber, Constant.defaultPageSize, '');
                setShowDelete(false);
            } else {
    ...
    return (
        <Card className="shadow mb-4">
            <Card.Header className="py-3">
                <h6 className="m-0 font-weight-bold text-primary">Tipos_Caracteristicas List ({rData.totalCount})
