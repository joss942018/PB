...
import { resetModel_AñoToInit, setModel_AñoMessage } from "redux/actions";
import { Button, Card, Col, Form, InputGroup } from "react-bootstrap";
import { Constant } from "template/Constant";
import ConfirmationModal from "template/ConfirmationModal";
import { deleteModel_Año } from "services/model_añoService";
type Props = {
    hideShowForm: (action) => void;
    handleRowEdit: (row) => void;
    getData: (page, pageSize, searchKey) => void;
};
export const Model_AñoTable: React.FC<Props> = ({ hideShowForm, handleRowEdit, getData }) => {
    ...
    const handleServerDelete = async () => {
        if (rowData) {
            const response = await deleteModel_Año(rowData.id);
            if (response) {
                dispatch(resetModel_AñoToInit());
                dispatch(setModel_AñoMessage("Deleted Successfully"));
                getData(Constant.defaultPageNumber, Constant.defaultPageSize, '');
                setShowDelete(false);
            } else {
...
{name: 'año_id', selector: row => row.año_id, sortable: true},
{name: 'descripcion', selector: row => row.año.descripcion, sortable: true},
{name: 'modelo_id', selector: row => row.modelo_id, sortable: true},
{name: 'nombre', selector: row => row.modelos.nombre, sortable: true},
    ...
    return (
        <Card className="shadow mb-4">
            <Card.Header className="py-3">
                <h6 className="m-0 font-weight-bold text-primary">Model_Año List ({rData.totalCount})
