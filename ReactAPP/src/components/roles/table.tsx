...
export const RolesTable: React.FC<Props> = ({ hideShowForm, handleRowEdit, getData }) => {
    ...
    const handleServerDelete = async () => {
        if (rowData) {
            const response = await deleteRoles(rowData.id);
            if (response) {
                dispatch(resetRolesToInit());
                dispatch(setRolesMessage("Deleted Successfully"));
                getData(Constant.defaultPageNumber, Constant.defaultPageSize, '');
                setShowDelete(false);
            } else {
    ...
    return (
        <Card className="shadow mb-4">
            <Card.Header className="py-3">
                <h6 className="m-0 font-weight-bold text-primary">Roles List ({rData.totalCount})
