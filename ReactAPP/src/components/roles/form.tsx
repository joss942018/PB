...
export const RolesForm: React.FC<Props> = ({ row, hideShowForm, getData, action }) => {
        ...
        onSubmit: async (values) => {
            if (action === 'edit') {
                ...
                if (response && response.data && response.data.code===1) {
                    dispatch(setRolesMessage("Updated Successfully"));
                    getData(Constant.defaultPageNumber, Constant.defaultPageSize, '');
                    hideShowForm('');
                } else {
                    dispatch(setRolesMessage("Some error occured!"));
                }
            } else if (action === 'add') {
                const response = await addRoles(values);
                if (response && response.data && response.data.code===1) {
                    dispatch(setRolesMessage("Añadido!"));
                    getData(Constant.defaultPageNumber, Constant.defaultPageSize, '');
                    hideShowForm('');
                } else {
                    dispatch(setRolesMessage("Some error occured!"));
    ...
    return (
        <Card className="shadow mb-4">
            <Card.Header className="py-3">
                <h6 className="m-0 font-weight-bold text-primary text-capitalize">{action} Roles
                    <Button className="btn-icon-split float-right" onClick={() => hideShowForm(false)}>
                        <span className="icon text-white-50">
                            <i className="fas fa-list"></i>
                        </span>
                        <span className="text">View Roles</span>
                    </Button>
                </h6>
